...
public class PathNode 
{
    ...
    public static PathNode GetNewNode(Vector3 position, float g_cost, float h_cost, NodeState state = NodeState.OPEN)
    {
        PathNode ret = new PathNode(position, g_cost, h_cost, state);
        return ret;
    }

    PathNode(Vector3 position, float g_cost, float h_cost, NodeState state)
    {
    ...
    /// <param name="state">The current state of the Node</param>
    public static PathNode GetNewNode(Vector3 position, float g_cost, float h_cost, PathNode previous, NodeState state = NodeState.OPEN)
    {
        PathNode ret = new PathNode(position, g_cost, h_cost, previous, state);
        return ret;
    }

    PathNode(Vector3 position, float g_cost, float h_cost, PathNode previous, NodeState state)
    {
        this.position = position;
        this.g_cost = g_cost;
        this.h_cost = h_cost;
