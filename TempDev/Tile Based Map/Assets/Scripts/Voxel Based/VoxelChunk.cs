...
{
    public static readonly int width = 16, height = 32, length = 16;

    public static VoxelChunk CreateChunk(Vector3Int position, Voxel[,,] voxels, Transform parent, Material material)
    {
        ...
        VoxelChunk chunk = Instantiate(WorldController.main.defaultVoxel, parent);
    ...
    //Fields
    Vector3Int position;
    public Voxel[,,] voxels = new Voxel[width, height, length];
    ...
    public void SetChunk(Vector3Int position, Voxel[,,] voxels, Material material)
    {
        if (voxels.GetLength(0) != width || voxels.GetLength(1) != height || voxels.GetLength(2) != length)
        {
            Debug.LogError("Cannot create chunk that doesn't conform to chunk width of " + width + " height of " + height + " and length of " + length);
    ...
    public void ReloadMesh()
    {
        ...
        void DrawVoxel(Vector3Int pos)
        {
            ...
            for(int i = 0; i < 6; i++)
            {
                ...
                {
                    Vector2 UVOffsett = (new Vector2(GetVoxel(pos).blockID, 0));
                    ...
                    uvs.Add((VoxelData.UVs[0] + UVOffsett) * TextureAtlasManager.NormalizedBlockTextureSize );
                    uvs.Add((VoxelData.UVs[1] + UVOffsett) * TextureAtlasManager.NormalizedBlockTextureSize);
                    uvs.Add((VoxelData.UVs[2] + UVOffsett) * TextureAtlasManager.NormalizedBlockTextureSize);
                    uvs.Add((VoxelData.UVs[3] + UVOffsett) * TextureAtlasManager.NormalizedBlockTextureSize);
                    triangles.Add(index);
    ...
    public Voxel GetVoxel(Vector3Int pos)
    {
        if(pos.x > width || pos.y > height || pos.z > length)
        {
            ...
            return new Voxel(-1, Vector3Int.zero);
        }
